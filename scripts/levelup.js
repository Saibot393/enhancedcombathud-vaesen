...
class gainXPWindow extends Application {
	...
	async getHTML(pOptions={}) {
		let entriesHTML = `<span>${game.i18n.localize(ModuleName + ".Titles.CheckWhichApplies")}</span>`;
		
		for (const key of Object.keys(this.XPOptions)) {
			entriesHTML = entriesHTML + 	`<div class="XPoption" name="${key}">
		...
		//buttons	
		let buttonsHTML = 				`<div class="form-group" style="display:flex;flex-direction:column;align-items:center;gap:1em;margin-top:1em">
											<button type="button" name="confirmXP"> <i class="${ConfirmIcon}"></i> ${game.i18n.localize(ModuleName + ".Titles.confirmXP")} </button>
										</div>`;
										
		return entriesHTML + buttonsHTML;
	}
...
class spendXPWindow extends Application {
	constructor(actor, Options = {}) {
		...
		this.xpavailable = this.actor.system.experience;
		...
		for (let key of Object.keys(this.actor.system.skill)) {
			skills[key] = this.actor.system.skill[key].value;
	...
	async getHTML(pOptions={}) {
		...
		for (const key of Object.keys(skills)) {
			entriesHTML = entriesHTML + 	`<div class="skillchoice" name="${key}" style="display:flex;margin-top:3px;margin-bottom:3px">
												<label style="width:50%">${game.i18n.localize(CONFIG.vaesen.skills[key])}</label>
	...
	async applyChoices() {
		if (await this.calculateXPleft() >= 0) {
			...
			//skills
			let skillupdates = {system : {skill : {}}};
			for (let key of Object.keys(this.skillincreases)) {
				skillupdates.system.skill[key] = {value : this.originalSkills[key] + this.skillincreases[key]};
			}
			
			this.actor.update(skillupdates);
			
			//XP
			this.actor.update({system : {experience : this.actor.system.experience - (await this.calculateXPused())}});
		}
	}
...
function xpThreshholdReached(actor) {
	return actor.system.experience >= Math.min(skillcost, talentcost);
}
